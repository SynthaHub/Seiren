...
scripts/             sitemap, brand assets, image resizing
...
that is almost always where to go.
...
| Item | Where | Note |
|---|---|---|
| **Telephone number** | `content/navigation.ts` | Supplied as `0180357040`. Kenya has no 018x mobile range — 07xx, or 010x/011x/015x. Almost certainly a transcription slip. Shipped exactly as given rather than guessed at. |
...
| **Final domain** | `lib/seo.ts` | `seiranpartners.co.ke` is a placeholder. Canonical URLs and every sitemap entry are built from it. |
| **FAQ answers** | `content/faqs.ts` | Drafts. Eleven questions, all emitted as FAQPage structured data, so they reach search results. |
...
| **Facebook URL** | `content/navigation.ts` | A `/share/` redirect rather than a canonical page URL. |
